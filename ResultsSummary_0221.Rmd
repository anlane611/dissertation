...
2. EM max.iter and tol parameters
3. TOAST results
4. EM results
...
I wanted to evaluate the efficiency and accuracy of the EM algorithm to set the tolerance and maximum iteration parameters. Up to this point, I had set max.iter=500 and tol=0.001.

```{r EMtest, echo=FALSE}
setwd("/Users/andrealane/Dropbox/CelltypeSpecificMediation/Simulation")

load("LikelihoodandEstimatePlotCode.RData")

mytabmat <- matrix(c(test[500,2:5],test2[500,2:5],test5[2000,2:5],test3[157,2:5],test4[500,2:5],test6[1937,2:5]),nrow=6,byrow = TRUE)

rownames(mytabmat) <- c("500; .01","500; .001","2000; .0001","500; .01","500; .001","2000; .0001")
kable(mytabmat,caption="Final iteration Theta2 estimates (true values in parentheses)",col.names=c("Cell 1 (0)","Cell 2 (0)","Cell 3 (0)","Cell 4 (0.4)")) %>% 
        kable_styling(latex_options = c("hold_position")) %>%
        pack_rows("Initial values: true values", 1, 3) %>%
        pack_rows("Initial values: 0", 4, 6)

```

Comments:

 - odd that convergence is faster with 0s as initial values than true values as initial values?
 - estimates not as good with 0s as initial values (to be expected)
 - have been using max.iter=500, tol=.001
...
getres <- function(folder,method,Ncell,Nmed,medpi,Nmedsites,theta){
        ...
        for(i in 1:10){
                new <- read.table(paste0("/Users/andrealane/Dropbox/CelltypeSpecificMediation/Simulation/",folder,"/",method,"_Ncell",Ncell,"_Nmed",Nmed,"_medpi",medpi,"_Nmedsites",Nmedsites,"_theta",theta,"_",i))
        mat <- mat + new
        }
        mat <- apply(mat,2,rev)
        return(mat)
...
For reference, I also present the power and type 1 error from the last set of results where I used 0 for the initial theta values.
