...
**Data generation**:
 ...
 - $\tau_k^2 = \sigma^2 = 0.01$ (unless otherwise noted, as in table 4 and figure 4)
...
getpvalmat <- function(folder,Ncell,medpi,Nsam,method,BinSig=FALSE){
  mymat <- c()
  for(i in 1:10){
    for(j in 1:10){
      newmat <- read.table(paste0(folder,"/",method,"_N",Nsam,"_Ncell",Ncell,"_medpi",medpi,"_sim",i,"_iseed",j))
      if(method=="MICS_Pvals"){newmat <- t(newmat)}
      newmat <- as.numeric(newmat)
      mymat <- rbind(mymat,newmat)
    }
  }
  if(BinSig) mymat <- colSums(ifelse(mymat<.05,1,0))
  return(mymat)
}
...
makeroc <- function(folder,Ncell,medpi,Nsam,leg=FALSE,title){
  colvec <- c("#000000", "#E69F00", "#56B4E9", "#009E73", 
                       "#0072B2", "#D55E00", "#CC79A7")
  pvals1 <- getpvalmat(folder,Ncell,medpi,Nsam,"EM_toastPvals")
  pvals2 <- getpvalmat(folder,Ncell,medpi,Nsam,"TCA_Pvals")
  pvals3 <- getpvalmat(folder,Ncell,medpi,Nsam,"MICS_Pvals")
  labmat <- getlabmat(Ncell,medpi)
  
  # preds <- cbind(pvals1,pvals2,pvals3)
  # labs <- cbind(labmat,labmat,labmat)
  ...
  # pred.mat <- prediction(as.numeric(preds),labels=as.numeric(labs))
  # perf.mat <- performance(pred.mat, "tpr", "fpr")
  # rocplot <- plot(perf.mat, col=as.list(colvec[1:3]))

  perf1 <- performance(pred1,measure="tpr",x.measure="fpr")
  perf2 <- performance(pred2,measure="tpr",x.measure="fpr")
  perf3 <- performance(pred3,measure="tpr",x.measure="fpr")

  rocplot <- plot(perf1, col=colvec[1],xaxis.cex.axis=0.8,yaxis.cex.axis=0.8,
                  main=title)
            ...
            if(leg){legend("bottomright",legend=c("EM-TO","TCA","MICS"),col=colvec[1:3],lty=1)}
...
```{r typ1plot2, echo=FALSE, results='hide'}
...
```{r ROC, echo=FALSE, results='hide'}

par(mfrow=c(2,2), mai=c(0.8,0.8,0.15,0))
makeroc("ROC_SD0.1",3,1,500,leg=TRUE,title="Cell 1 mediator")
makeroc("ROC_SD0.1",3,2,500,title="Cell 2 mediator")
makeroc("ROC_SD0.1",3,3,500,title="Cell 3 mediator")

```
**Figure 3**: ROC curves for 3 cell types, N=500, sigma = tau = 0.1 (results correspond to table 2)

\newpage

```{r ROC2, echo=FALSE, results='hide'}

par(mfrow=c(2,2), mai=c(0.8,0.8,0.15,0))
makeroc("ROC_SD1",3,1,500,leg=TRUE,title="Cell 1 mediator")
makeroc("ROC_SD1",3,2,500,title="Cell 2 mediator")
makeroc("ROC_SD1",3,3,500,title="Cell 3 mediator")


```

**Figure 4**: ROC curves for 3 cell types, N=500, sigma = tau = 1 (results correspond to table 4)
