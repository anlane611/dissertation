...
**Data generation**:
 ...
 - Methylation values generated from beta distribution using BloodPureDNAmethy_4ct.rda values for mean and sd
 ...
 - exposure-mediator effect induced by multiplying cell-type-specific methylation value in exposed by $\beta_1$
 ...
 - $\beta_1 = 1.2$
 - $\theta_2 = 0.2$
 - $\gamma^2 = SD(Y_1)/2$, where $Y_1$ is the originally generated outcome
 - $\tau_k^2 = \sigma^2 = 0.01$ OR $1$
...
getpvalmat <- function(folder,Ncell,medpi,Nsam,method,sigma,tau,gammadenom){
  mymat <- c()
    for(j in 1:10){
  ...
  alpha <- ifelse(method=="MICS_Pvals",0.05,.05/Ncell)
  binmat <- ifelse(mymat<alpha,1,0)
...
makeroc <- function(folder,Ncell,medpi,Nsam,sigma,tau,gammadenom=2,leg=FALSE,title){
  ...
  pvals1 <- getpvalmat(folder,Ncell,medpi,Nsam,"EM_toastPvals",sigma,tau,gammadenom)$mymat
  ...
  pvals4 <- getpvalmat(folder,Ncell,medpi,Nsam,"EM_truePvals",sigma,tau,gammadenom)$mymat
  pvals5 <- getpvalmat(folder,Ncell,medpi,Nsam,"EM_randPvals",sigma,tau,gammadenom)$mymat
  ...
  rocplot <- plot(perf1, col=colvec[1],
            ...
            if(leg){legend("bottomright",legend=c("EM-Tr","EM-R","EM-TO","TCA","MICS"),col=colvec[c(4,5,1,2,3)],lty=1)}
            abline(a=0, b= 1)

  return(rocplot)
}

type1tab <- function(tabmat){
        Ncell <- ncol(tabmat)/3
        mytab <- matrix(NA,nrow=5*Ncell,ncol=3)
        for(i in 1:3){ #loop over sample sizes
                for(j in 1:Ncell){ #loop over mediating cells
                       mytab[(5*(j-1))+(1:5),i] <- rowMeans(tabmat[,(i-1)*(Ncell)+(1:Ncell)][(5*(j-1))+(1:5),-j]) 
                }
        } 
        return(mytab/100)
}

type1plot <- function(Nsam,Ncell,tabmat,nmethod=5,text=FALSE,ylab=FALSE,xlab=FALSE,yax=FALSE,title=FALSE,leg=FALSE){
        colvec <- c("#000000", "#E69F00", "#56B4E9", "#009E73", 
...
powerplot <- function(Nsam,Ncell,tabmat,text=FALSE,ylab=FALSE,xlab=FALSE,yax=FALSE,title=FALSE){
        colvec <- c("#000000", "#E69F00", "#56B4E9", "#009E73", 
                       "#0072B2", "#D55E00", "#CC79A7") #colorblind friendly
        pchvec <- c(15,17:20)
        ind <- matrix(c(1,1,6,2,11,3,16,4),nrow=4,ncol=2,byrow=TRUE)
...
myrows <- c("EM - True","EM - Random","EM - TOAST",
                             "TCA","MICS",
                             "EM - True ","EM - Random ","EM - TOAST ",
                             "TCA ","MICS ",
                             "EM - True  ","EM - Random  ","EM - TOAST  ",
                             "TCA  ","MICS  ",
                             "EM - True   ","EM - Random   ","EM - TOAST   ",
                             ...
                             "EM - True       ","EM - Random       ","EM - TOAST       ",
                             "TCA       ","MICS       ")
rownames(tabmat.2cells) <- myrows[1:10]
rownames(tabmat.3cells) <- myrows[1:15]
rownames(tabmat.4cells) <- myrows[1:20]


gettabmat2 <- function(Ncell,noise1,noise2){
  sam <- c(200,500,1000)
  methods <- c("EM_truePvals","EM_randPvals","EM_toastPvals","TCA_Pvals","MICS_Pvals")
  mymat <- matrix(NA,nrow=Ncell*length(methods),ncol=Ncell*length(sam))
  for(k in sam){
    for(i in 1:Ncell){
      for(j in methods){
        mymat[seq(which(methods==j),Ncell*length(methods),length(methods))[i],(which(sam==k)*Ncell-(Ncell-1)):(Ncell*which(sam==k))] <- colSums(getpvalmat("050221",Ncell,i,k,
                                         j,noise1,noise1,noise2)$binmat)
      }
    }
  }
  return(mymat)
...
tabmat2 <- rbind(gettabmat2(2,0.03,5),gettabmat2(2,0.1,5))
...
kable(tabmat2,caption="Significant cell types out of 100 replicates - 2 cells (.4, .6)",col.names=c("Cell 1","Cell 2","Cell 1","Cell 2","Cell 1","Cell 2"),booktabs=T)%>% 
        ...
        column_spec(c(2,4,6), bold = c(T,T,T,T,T,F,F,F,F,F,T,T,T,T,T,F,F,F,F,F))%>% 
        column_spec(c(3,5,7), bold = c(F,F,F,F,F,T,T,T,T,T,F,F,F,F,F,T,T,T,T,T))%>%
        add_header_above(c(" "=1,"N=200"=2,"N=500"=2,"N=1000"=2)) %>% 
        pack_rows("Small sigma/tau (0.1)", 1, 10) %>% 
        pack_rows("Big sigma/tau (1)", 11, 20)

tabmat3 <- rbind(gettabmat2(3,0.03,5),gettabmat2(3,0.1,5))
...
kable(tabmat3,caption="Significant cell types out of 100 replicates - 3 cells (.2, .3, .5)",col.names=c("Cell 1","Cell 2","Cell 3","Cell 1","Cell 2","Cell 3","Cell 1","Cell 2","Cell 3"),booktabs=T)%>%
        kable_styling(latex_options = c("hold_position")) %>%
        column_spec(c(2,5,8), bold = c(T,T,T,T,T,F,F,F,F,F,F,F,F,F,F,
                                       T,T,T,T,T,F,F,F,F,F,F,F,F,F,F))%>%
        column_spec(c(3,6,9), bold = c(F,F,F,F,F,T,T,T,T,T,F,F,F,F,F,
                                       F,F,F,F,F,T,T,T,T,T,F,F,F,F,F))%>%
        column_spec(c(4,7,10), bold = c(F,F,F,F,F,F,F,F,F,F,T,T,T,T,T,
                                        F,F,F,F,F,F,F,F,F,F,T,T,T,T,T))%>%
        add_header_above(c(" "=1,"N=200"=3,"N=500"=3,"N=1000"=3)) %>%
        pack_rows("Small sigma/tau (0.1)", 1, 15) %>%
        pack_rows("Big sigma/tau (1)", 16, 30)

```


```{r newtabs2, echo=FALSE}
tabmat4 <- rbind(gettabmat2(4,0.03,5),gettabmat2(4,0.1,5))
...
kable(tabmat4,caption="Significant cell types out of 100 replicates - 4 cells (.1, .2, .3, .4)",col.names=c("Cell 1","Cell 2","Cell 3","Cell 4","Cell 1","Cell 2","Cell 3","Cell 4","Cell 1","Cell 2","Cell 3","Cell 4"),booktabs=T)%>% 
        kable_styling(latex_options = c("hold_position")) %>% 
        column_spec(c(2,6,10), bold = c(T,T,T,T,T,F,F,F,F,F,F,F,F,F,F,F,F,F,F,F,
                                        T,T,T,T,T,F,F,F,F,F,F,F,F,F,F,F,F,F,F,F))%>%
        column_spec(c(3,7,11), bold = c(F,F,F,F,F,T,T,T,T,T,F,F,F,F,F,F,F,F,F,F,
                                        F,F,F,F,F,T,T,T,T,T,F,F,F,F,F,F,F,F,F,F))%>%
        column_spec(c(4,8,12), bold = c(F,F,F,F,F,F,F,F,F,F,T,T,T,T,T,F,F,F,F,F,
                                        F,F,F,F,F,F,F,F,F,F,T,T,T,T,T,F,F,F,F,F))%>%
        column_spec(c(5,9,13), bold = c(F,F,F,F,F,F,F,F,F,F,F,F,F,F,F,T,T,T,T,T,
                                        F,F,F,F,F,F,F,F,F,F,F,F,F,F,F,T,T,T,T,T))%>%
        add_header_above(c(" "=1,"N=200"=4,"N=500"=4,"N=1000"=4)) %>% 
        pack_rows("Small sigma/tau (0.1)", 1, 20) %>% 
        pack_rows("Big sigma/tau (1)", 21, 40)
...
makeroc("050221",2,1,200,0.03,0.03,gammadenom=5,leg=TRUE,title="N=200,cell 1 mediator")
makeroc("050221",2,2,200,0.03,0.03,gammadenom=5,title="N=200,cell 2 mediator")
makeroc("050221",2,1,500,0.03,0.03,gammadenom=5,title="N=500,cell 1 mediator")
makeroc("050221",2,2,500,0.03,0.03,gammadenom=5,title="N=500,cell 2 mediator")
makeroc("050221",2,1,1000,0.03,0.03,gammadenom=5,title="N=1000,cell 1 mediator")
makeroc("050221",2,2,1000,0.03,0.03,gammadenom=5,title="N=1000,cell 2 mediator")


```
**Figure 1**: 2 cell types, Low noise (sigma=tau=0.03, gamma denominator=5)
...
par(mfrow=c(3,2), mai=c(0.1,0.1,0.1,0))
makeroc("050221",2,1,200,0.1,0.1,leg=TRUE,title="N=200,cell 1 mediator")
makeroc("050221",2,2,200,0.1,0.1,title="N=200,cell 2 mediator")
makeroc("050221",2,1,500,0.1,0.1,title="N=500,cell 1 mediator")
makeroc("050221",2,2,500,0.1,0.1,title="N=500,cell 2 mediator")
makeroc("050221",2,1,1000,0.1,0.1,title="N=1000,cell 1 mediator")
makeroc("050221",2,2,1000,0.1,0.1,title="N=1000,cell 2 mediator")

```
**Figure 2**: 2 cell types, High noise (sigma=tau=0.1, gamma denominator=2)


```{r 3cellROCsmallsd, echo=FALSE, results='hide'}
par(mfrow=c(3,3), mai=c(0.1,0.1,0.1,0))
makeroc("050221",3,1,200,0.03,0.03,gammadenom=5,leg=TRUE,title="N=200,cell 1 mediator")
makeroc("050221",3,2,200,0.03,0.03,gammadenom=5,title="N=200,cell 2 mediator")
makeroc("050221",3,3,200,0.03,0.03,gammadenom=5,title="N=200,cell 3 mediator")
makeroc("050221",3,1,500,0.03,0.03,gammadenom=5,title="N=500,cell 1 mediator")
makeroc("050221",3,2,500,0.03,0.03,gammadenom=5,title="N=500,cell 2 mediator")
makeroc("050221",3,3,500,0.03,0.03,gammadenom=5,title="N=500,cell 3 mediator")
makeroc("050221",3,1,1000,0.03,0.03,gammadenom=5,title="N=1000,cell 1 mediator")
makeroc("050221",3,2,1000,0.03,0.03,gammadenom=5,title="N=1000,cell 2 mediator")
makeroc("050221",3,3,1000,0.03,0.03,gammadenom=5,title="N=1000,cell 3 mediator")

```
**Figure 3**: 3 cell types, Low noise (sigma=tau=0.03, gamma denominator=5)

```{r 3cellROCbigsd, echo=FALSE, results='hide'}
par(mfrow=c(3,3), mai=c(0.1,0.1,0.1,0))
makeroc("050221",3,1,200,0.1,0.1,leg=TRUE,title="N=200,cell 1 mediator")
makeroc("050221",3,2,200,0.1,0.1,title="N=200,cell 2 mediator")
makeroc("050221",3,3,200,0.1,0.1,title="N=200,cell 3 mediator")
makeroc("050221",3,1,500,0.1,0.1,title="N=500,cell 1 mediator")
makeroc("050221",3,2,500,0.1,0.1,title="N=500,cell 2 mediator")
makeroc("050221",3,3,500,0.1,0.1,title="N=500,cell 3 mediator")
makeroc("050221",3,1,1000,0.1,0.1,title="N=1000,cell 1 mediator")
makeroc("050221",3,2,1000,0.1,0.1,title="N=1000,cell 2 mediator")
makeroc("050221",3,3,1000,0.1,0.1,title="N=1000,cell 3 mediator")

```
**Figure 4**: 3 cell types, High noise (sigma=tau=0.1, gamma denominator=2)


```{r 4cellROCsmallsd, echo=FALSE, results='hide'}
par(mfrow=c(3,4), mai=c(0.1,0.1,0.1,0))
makeroc("050221",4,1,200,0.03,0.03,gammadenom=5,leg=TRUE,title="N=200,cell 1 mediator")
makeroc("050221",4,2,200,0.03,0.03,gammadenom=5,title="N=200,cell 2 mediator")
makeroc("050221",4,3,200,0.03,0.03,gammadenom=5,title="N=200,cell 3 mediator")
makeroc("050221",4,4,200,0.03,0.03,gammadenom=5,title="N=200,cell 4 mediator")
makeroc("050221",4,1,500,0.03,0.03,gammadenom=5,title="N=500,cell 1 mediator")
makeroc("050221",4,2,500,0.03,0.03,gammadenom=5,title="N=500,cell 2 mediator")
makeroc("050221",4,3,500,0.03,0.03,gammadenom=5,title="N=500,cell 3 mediator")
makeroc("050221",4,4,500,0.03,0.03,gammadenom=5,title="N=500,cell 4 mediator")
makeroc("050221",4,1,1000,0.03,0.03,gammadenom=5,title="N=1000,cell 1 mediator")
makeroc("050221",4,2,1000,0.03,0.03,gammadenom=5,title="N=1000,cell 2 mediator")
makeroc("050221",4,3,1000,0.03,0.03,gammadenom=5,title="N=1000,cell 3 mediator")
makeroc("050221",4,4,1000,0.03,0.03,gammadenom=5,title="N=1000,cell 4 mediator")

```
**Figure 5**: 4 cell types, Low noise (sigma=tau=0.03, gamma denominator=5)


```{r 4cellROCbigsd, echo=FALSE, results='hide'}
par(mfrow=c(3,4), mai=c(0.1,0.1,0.1,0))
makeroc("050221",4,1,200,0.1,0.1,leg=TRUE,title="N=200,cell 1 mediator")
makeroc("050221",4,2,200,0.1,0.1,title="N=200,cell 2 mediator")
makeroc("050221",4,3,200,0.1,0.1,title="N=200,cell 3 mediator")
makeroc("050221",4,4,200,0.1,0.1,title="N=200,cell 4 mediator")
makeroc("050221",4,1,500,0.1,0.1,title="N=500,cell 1 mediator")
makeroc("050221",4,2,500,0.1,0.1,title="N=500,cell 2 mediator")
makeroc("050221",4,3,500,0.1,0.1,title="N=500,cell 3 mediator")
makeroc("050221",4,4,500,0.1,0.1,title="N=500,cell 4 mediator")
makeroc("050221",4,1,1000,0.1,0.1,title="N=1000,cell 1 mediator")
makeroc("050221",4,2,1000,0.1,0.1,title="N=1000,cell 2 mediator")
makeroc("050221",4,3,1000,0.1,0.1,title="N=1000,cell 3 mediator")
makeroc("050221",4,4,1000,0.1,0.1,title="N=1000,cell 4 mediator")

```
**Figure 6**: 4 cell types, High noise (sigma=tau=0.1, gamma denominator=2)

\newpage
\section{Other meeting notes}

**Real data**: looked at exposure-outcome univariate relationships with the given variables. Did not find significant relationship between trauma and any of the hormone/biological outcomes. Did find significant relationship between smoking and weight, which seems like a reasonable relationship to test for methylation mediation?

**Met with David about mediation**: talked about structure of paper in terms of causal inference. Product of coefficients method can have a causal interpretation when certain assumptions are met. He suggested I spend time looking at literature and evaluating if these assumptions are biologically reasonable. Focus of BIG presentation?

Assumptions:

 - no exposure-mediator interaction (could be added in model)
 ...
 - no mediator-outcome confounder that is affected by the exposure (nothing on pathway from exposure to mediator that also affects outcome)
 - temporal ordering
...
tab.2 <- rbind(tabmat2[1:5,c(2,4,6)],
               setNames(tabmat2[6:10,c(1,3,5)], names(tabmat2[6:10,c(2,4,6)])))/100
tab.3 <- type1tab(tabmat3)
tab.4 <- type1tab(tabmat4)
...
par(mfrow=c(3,3), mai=c(0.3,0.3,0.15,0))
powerplot(200,2,tabmat2,ylab=TRUE,yax=TRUE,text=TRUE,title=TRUE)
powerplot(500,2,tabmat2,title=TRUE)
powerplot(1000,2,tabmat2,title=TRUE)
powerplot(200,3,tabmat3,yax=TRUE,ylab=TRUE)
powerplot(500,3,tabmat3)
powerplot(1000,3,tabmat3)
powerplot(200,4,tabmat4,xlab=TRUE,yax=TRUE,ylab=TRUE)
powerplot(500,4,tabmat4,xlab=TRUE)
powerplot(1000,4,tabmat4,xlab=TRUE)

```
<!-- **Figure 2**: Power for all methods across sample sizes and number of cell types. The number plotted is the number of times, out of 100 replicates, that that cell type was correctly identified as a mediator. Note that only one cell type is a mediator at a time. -->

```{r tabs, echo=FALSE}
kable(tabmat.2cells,caption="Significant cell types out of 100 replicates - 2 cells (.4, .6)",col.names=c("Cell 1","Cell 2","Cell 1","Cell 2","Cell 1","Cell 2"),booktabs=T)%>% 
        kable_styling(latex_options = c("hold_position")) %>% 
        column_spec(c(2,4,6), bold = c(T,T,T,T,T,F,F,F,F,F))%>% 
        column_spec(c(3,5,7), bold = c(F,F,F,F,F,T,T,T,T,T))%>%
        add_header_above(c(" "=1,"N=200"=2,"N=500"=2,"N=1000"=2))

kable(tabmat.3cells,caption="Significant cell types out of 100 replicates - 3 cells (.2, .3, .5)",col.names=c("Cell 1","Cell 2","Cell 3","Cell 1","Cell 2","Cell 3","Cell 1","Cell 2","Cell 3"),booktabs=T)%>% 
        kable_styling(latex_options = c("hold_position")) %>% 
        column_spec(c(2,5,8), bold = c(T,T,T,T,T,F,F,F,F,F,F,F,F,F,F))%>% 
        column_spec(c(3,6,9), bold = c(F,F,F,F,F,T,T,T,T,T,F,F,F,F,F))%>%
        column_spec(c(4,7,10), bold = c(F,F,F,F,F,F,F,F,F,F,T,T,T,T,T))%>%
...
```{r tabs2, echo=FALSE}
kable(tabmat.4cells,caption="Significant cell types out of 100 replicates - 4 cells (.1, .2, .3, .4)",col.names=c("Cell 1","Cell 2","Cell 3","Cell 4","Cell 1","Cell 2","Cell 3","Cell 4","Cell 1","Cell 2","Cell 3","Cell 4"),booktabs=T)%>% 
        kable_styling(latex_options = c("hold_position")) %>% 
        column_spec(c(2,6,10), bold = c(T,T,T,T,T,F,F,F,F,F,F,F,F,F,F,F,F,F,F,F))%>%
        column_spec(c(3,7,11), bold = c(F,F,F,F,F,T,T,T,T,T,F,F,F,F,F,F,F,F,F,F))%>%
        column_spec(c(4,8,12), bold = c(F,F,F,F,F,F,F,F,F,F,T,T,T,T,T,F,F,F,F,F))%>%
...
# tabmat.3cells.SD1 <- rbind(
#   getpvalmat("ROC_SD1",3,1,500,"EM_toastPvals",BinSig=TRUE),
#   getpvalmat("ROC_SD1",3,1,500,"TCA_Pvals",BinSig=TRUE),
#   getpvalmat("ROC_SD1",3,1,500,"MICS_Pvals",BinSig=TRUE),
#   getpvalmat("ROC_SD1",3,2,500,"EM_toastPvals",BinSig=TRUE),
#   getpvalmat("ROC_SD1",3,2,500,"TCA_Pvals",BinSig=TRUE),
#   getpvalmat("ROC_SD1",3,2,500,"MICS_Pvals",BinSig=TRUE),
#   getpvalmat("ROC_SD1",3,3,500,"EM_toastPvals",BinSig=TRUE),
#   getpvalmat("ROC_SD1",3,3,500,"TCA_Pvals",BinSig=TRUE),
#   getpvalmat("ROC_SD1",3,3,500,"MICS_Pvals",BinSig=TRUE)
# )

# rownames(tabmat.3cells.SD1) <- myrows[c(3:5,8:10,13:15)]
# 
# 
# kable(tabmat.3cells.SD1,caption="Significant cell types out of 100 replicates - 3 cells (.2, .3, .5) \n sigma=tau=1",col.names=c("Cell 1","Cell 2","Cell 3"),booktabs=T, linesep = linesep(c(3,3))) %>% 
#         kable_styling(latex_options = c("hold_position")) %>% 
#         column_spec(2, bold = c(T,T,T,F,F,F,F,F,F))%>% 
#         column_spec(3, bold = c(F,F,F,T,T,T,F,F,F))%>%
#         column_spec(4, bold = c(F,F,F,F,F,F,T,T,T))%>%
#         add_header_above(c(" "=1,"N=500"=3))
