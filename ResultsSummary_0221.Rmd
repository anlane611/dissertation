...
Same results as above but in table form
...
\section{Comments for discussion}

1. It looks like the EM with TOAST initial values works pretty well. It has inflated type 1 error in some cases though, especially with 2 cell types. I assume these are the cases where TOAST does not detect a significant site so 0 is used as the initial value for all cell types. But in most cases, it controls type 1 error and is still more powerful than TCA and MICS.

2. I would expect power to increase with both sample size and cell proportion, but this is not always the case, especially in 2 cell types. I don't know why this is the case.

3. I am surprised by the big increase in power in TCA and MICS for cell type 4 compared to 1, 2, and 3 when there are 4 cell types. I would expect more of a constant increase as the cell proportion increases. Instead, it is very low for cell types 1-3 and then very high for cell type 4.

4. Interesting that EM - Random error is so high.