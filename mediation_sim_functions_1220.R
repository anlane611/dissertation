...
getProportion <- function(N_sample, cc = 100, E.exp, E.unexp, med.pi,
  ...
  prop.matrix.ctr = matrix(0,N_sample,length(alpha.ctr))
    if (med){
      ...
      prop.matrix.ctr[X2.1,med.pi] = prop.matrix.ctr[X2.1,med.pi]*x2_med_effect
      prop.matrix.ctr[X2.1,nonmed.pi] = prop.matrix.ctr[X2.1,nonmed.pi] +
        (1-rowSums(prop.matrix.ctr)[1:length(X2.1)])/length(nonmed.pi)
...
julia_command("
    function IndefBoot(M, Y, E, beta0, beta1, prop, tausq,
        ...
        function getquant(indefb,Ncell)
            ...
            for i in 1:Ncell
                bootint[:,i] = quantile(myindefb[:,i],[.025/(Ncell*Ncpg),1-(.025/(Ncell*Ncpg))])
            end
            return bootint
        end
...
julia_command("
    ...
    for i in 1:Ncpg
          ...
          BootSig[i,:] = IndefBoot(M[i,:], Y, E, beta0, beta1, prop, tausq,
                       sigmasq, gammasq, theta0, theta1, theta2, Ncpg)[1]
    end
